...
var match_meshexactly='beginningmatch';
var jqxhr = null;
   ...
   temp=$('#mesh').val();
	if(temp.length!=0){
       pubmed_filter = 'has';
		...
		$('[name=pubmed_filter]').prop('disabled', true);
    ...
    $('[name=pubmed_filter][value=' + pubmed_filter + ']').prop('checked', true);
	 ...
	 $('[name=mesh_filter][value=' + mesh_filter + ']').prop('checked', true);
	  $('[name=match_meshexactly][value=' + match_meshexactly + ']').prop('checked', true);
...
.on('click', function () {
	 if(localStorage.getItem('popState') != 'shown'){
       ...
       alert('If a MeSH term is entered, only results with publications will be returned.');
	...
	temp=$(this).val();
	   ...
	   else
	   {
		 pubmed_filter = 'all';
		$('[name=pubmed_filter][value=' + pubmed_filter + ']').prop('checked', true);
		$('[name=pubmed_filter]').prop('disabled', false);
	   }   
...
function query(type, focus) {
    ...
    jqxhr = $.get('query.php', {
            ...
            mirna: $('#mirna').val(),
            mesh: $('#mesh').val(),
			match_meshexactly:$('[name=match_meshexactly]:checked').val()
        })
        .done(function (json) {
